...
module "azfun_dotnet" {
  ...
  name                                      = "azfun-dotnet-${var.project}-${var.organisation}-${var.environment}"
  resource_group_name                       = data.azurerm_resource_group.main.name
  location                                  = data.azurerm_resource_group.main.location
  storage_account_access_key                = module.azfun_dotnet_stor.primary_access_key
  app_service_plan_id                       = module.azfun_dotnet_plan.id
  storage_account_name                      = module.azfun_dotnet_stor.name
  ...
  dependencies                              = [
    ...
    module.azfun_dotnet_plan.dependent_on,
    module.azfun_dotnet_stor.dependent_on,
  ]
}

module "azfun_dotnet_plan" {
  source              = "git::https://github.com/Energinet-DataHub/geh-terraform-modules.git//app-service-plan?ref=1.2.0"
  name                = "asp-dotnet-${var.project}-${var.organisation}-${var.environment}"
  resource_group_name = data.azurerm_resource_group.main.name
  location            = data.azurerm_resource_group.main.location
  kind                = "FunctionApp"
  sku                 = {
